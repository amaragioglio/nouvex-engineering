...
The Scope Engine submits qualified project inquiries to `POST /api/leads`. The endpoint stores the inquiry in Supabase first, then sends a notification to `info@nouvexeng.com` through Resend.
...
4. Deploy the project to Vercel and submit a complete Scope Engine inquiry.
...
Run `node dev-server.js`, then open `http://127.0.0.1:4174/#scope`. Complete the Scope Engine and submit the brief. The demo keeps up to 50 test inquiries in memory at `http://127.0.0.1:4174/api/leads`; restarting the server clears them.
